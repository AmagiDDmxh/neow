import { Base58, ECPoint, ECDsa, RIPEMD160, Sha256, ECCurve, ECDsaCryptoKey } from '../neo/Cryptography'
...
export class Helper {
  ...
  public static GetPublicKeyFromPrivateKey (privateKey: Uint8Array): Uint8Array {
    var pkey = ECPoint.multiply(
      ECCurve.secp256r1.G, privateKey);
  ...
  public static Sign (message: Uint8Array, privateKey: Uint8Array): Uint8Array {
    ...
    var PublicKey = ECPoint.multiply(
      ECCurve.secp256r1.G, privateKey);
    var pubkey = PublicKey.encodePoint(false).subarray(1, 64);

    //var PublicKey = ThinECC.ECCurve.Secp256r1.G * prikey;
    ...
    ////using(var ecdsa = System.Security.Cryptography.ECDsa.Create(new
    // System.Security.Cryptography.ECParameters { Curve =
    // System.Security.Cryptography.ECCurve.NamedCurves.nistP256, D = prikey, Q
    // = new System.Security.Cryptography.ECPoint { X =
    // pubkey.Take(32).ToArray(), Y = pubkey.Skip(32).ToArray() } }))
    {
  ...
  public static VerifySignature (
    message: Uint8Array, signature: Uint8Array, pubkey: Uint8Array) {
    var PublicKey = ECPoint.decodePoint(
      pubkey, ECCurve.secp256r1);
    ...
    //using (System.Security.Cryptography.CngKey key =
    // System.Security.Cryptography.CngKey.Import(usepk,
    // System.Security.Cryptography.CngKeyBlobFormat.EccPublicBlob)) using
    // (System.Security.Cryptography.ECDsaCng ecdsa = new
    // System.Security.Cryptography.ECDsaCng(key))

    //using(var ecdsa = System.Security.Cryptography.ECDsa.Create(new
    // System.Security.Cryptography.ECParameters { Curve =
    // System.Security.Cryptography.ECCurve.NamedCurves.nistP256, Q = new
    // System.Security.Cryptography.ECPoint { X = usepk.Take(32).ToArray(), Y =
    // usepk.Skip(32).ToArray() } }))
    {
      //var hash = sha256.ComputeHash(message);
  ...
  public static Aes256Encrypt_u8 (
    src: Uint8Array, key: Uint8Array): Uint8Array {
    ...
    for (var i = 0; i < src.length / 4; i++) {
      srcs.words[i] = src[i * 4 + 3] + src[i * 4 + 2] * 256 +
        src[i * 4 + 1] * 256 * 256 + src[i * 4 + 0] * 256 * 256 * 256;
    }
    ...
    for (var i = 0; i < key.length / 4; i++) {
      keys.words[i] = key[i * 4 + 3] + key[i * 4 + 2] * 256 +
        key[i * 4 + 1] * 256 * 256 + key[i * 4 + 0] * 256 * 256 * 256;
    }
  ...
  public static Aes256Decrypt_u8 (
    encryptedkey: Uint8Array, key: Uint8Array): Uint8Array {
  ...
  public static GetNep2FromPrivateKey (
    prikey: Uint8Array, passphrase: string, n = 16384, r = 8, p = 8,
  ...
  public static GetPrivateKeyFromNep2 (
    nep2: string, passphrase: string, n = 16384, r = 8, p = 8,
