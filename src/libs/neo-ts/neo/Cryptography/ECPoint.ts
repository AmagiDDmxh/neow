...
export class ECPoint {
  constructor (
    public x: ECFieldElement, public y: ECFieldElement, public curve: ECCurve) {
    if ((x == null) != (y == null))
  ...
  public static decodePoint (encoded: Uint8Array, curve: ECCurve): ECPoint {
    ...
    switch (encoded[0]) {
      ...
      case 0x07: // hybrid
      {
        ...
        let X1 = BigInteger.fromUint8Array(
          encoded.subarray(1, 1 + expectedLength), 1, false);
        let Y1 = BigInteger.fromUint8Array(
          encoded.subarray(1 + expectedLength), 1, false);
        p = new ECPoint(
          new ECFieldElement(X1, curve), new ECFieldElement(Y1, curve), curve);
        break;
      }
      default:
  ...
  private static decompressPoint (
    yTilde: number, X1: BigInteger, curve: ECCurve): ECPoint {
  ...
  public static deserializeFrom (
    reader: IO.BinaryReader, curve: ECCurve): ECPoint {
  ...
  public static multiply (p: ECPoint, n: Uint8Array | BigInteger): ECPoint {
    let k = n instanceof Uint8Array
      ? BigInteger.fromUint8Array(n, 1, false)
      : n as BigInteger;
