...
export class CryptoKey {
  constructor (
    public type: string, public extractable: boolean,
    public algorithm: Algorithm, public usages: string[]
  ) {
  }
}
...
export class AesCryptoKey extends CryptoKey {
  constructor (private _key_bytes: Uint8Array) {
    super(
      "secret", true, { name: "AES-CBC", length: _key_bytes.length * 8 } as any,
      ["encrypt", "decrypt"]
    );
  }
  ...
  public static import (keyData: ArrayBuffer | ArrayBufferView): AesCryptoKey {
    if (keyData.byteLength != 16 && keyData.byteLength != 24 &&
      keyData.byteLength != 32)
      throw new RangeError();
...
export class ECDsaCryptoKey extends CryptoKey {
  constructor (public publicKey: ECPoint, public privateKey?: Uint8Array) {
    super(
      privateKey == null ? "public" : "private", true,
      { name: "ECDSA", namedCurve: "P-256" } as any,
      [privateKey == null ? "verify" : "sign"]
    );
  }
}
