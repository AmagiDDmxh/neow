...
export class MemoryStream extends Stream {
  ...
  public constructor (
    buffer: ArrayBuffer, index: number, count: number, writable?: boolean);
  public constructor () {
    super();
    if (arguments.length == 0) {
      ...
      if (arguments.length == 1) {
        ...
        this._writable = arguments.length == 4
          ? arguments[3] as boolean
          : false;
  ...
  private findBuffer (position: number): { iBuff: number, pBuff: number } {
    ...
    let firstSize = this._buffers[0] == null
      ? BufferSize
      : this._buffers[0].byteLength;
    ...
    if (this._expandable) {
      ...
      while (count > 0) {
        ...
        if (this._buffers[d.iBuff] == null) {
          ...
          actual_count = Math.min(
            count, this._buffers[d.iBuff].byteLength - d.pBuff);
  ...
  public setLength (value: number): void {
    if (value < 0 || (value != this._length && !this._writable) ||
      (value > this._capacity && !this._expandable))
      throw new RangeError();
    this._length = value;
  ...
  public toArray (): ArrayBuffer {
    if (this._buffers.length == 1 && this._origin == 0 &&
      this._length == this._buffers[0].byteLength)
  ...
  public write (buffer: ArrayBuffer, offset: number, count: number): void {
    if (!this._writable ||
      (!this._expandable && this._capacity - this._position < count))
      throw new Error();
    if (this._expandable) {
