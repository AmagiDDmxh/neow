...
export class Uint64 extends UintVariable {
  public static get MaxValue () {
    return _max || (_max = new Uint64(0xffffffff, 0xffffffff));
  }
  ...
  public leftShift (shift: number): Uint64 {
    ...
    for (let i = shift_units; i < bits.length; i++)
      if (shift == 0)
        ...
        bits[i] = this._bits[i - shift_units] << shift |
          this._bits[i - shift_units - 1] >>> (32 - shift);
    return new Uint64(bits[0], bits[1]);
  }
  ...
  public subtract (other: Uint64): Uint64 {
    let low = this._bits[0] - other._bits[0];
    let high = this._bits[1] - other._bits[1] -
      (this._bits[0] < other._bits[0] ? 1 : 0);
    return new Uint64(low, high);
  }
