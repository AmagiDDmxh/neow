...
export class Fixed8 implements IO.ISerializable {
  public static get MaxValue () {
    return _max || (_max = new Fixed8(new Uint64(0xffffffff, 0x7fffffff)));
  }

  public static get MinusOne () {
    return _minus || (_minus = new Fixed8(new Uint64(0xffffffff, 0xffffffff)));
  }

  public static get MinValue () {
    return _min || (_min = new Fixed8(Uint64.MinValue));
  }

  public static get One () { return _one || (_one = Fixed8.fromNumber(1)); }

  public static get Satoshi () {
    return _satoshi || (_satoshi = new Fixed8(new Uint64(1)));
  }
  ...
  constructor (private data: Uint64) {
    if (data.bits[1] >= 0x80000000 &&
      (data.bits[0] != 0xffffffff || data.bits[1] != 0xffffffff))
  ...
  public static fromNumber (value: number): Fixed8 {
    ...
    let array = new Uint32Array(
      (new BigInteger(value)).toUint8Array(true, 8).buffer);
