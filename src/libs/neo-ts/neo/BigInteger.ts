...
export class BigInteger {
  ...
  public static get MinusOne () {
    return _minusone || (_minusone = new BigInteger(-1));
  }
    ...
    if (typeof value === "number") {
      ...
      if (parts.exp <= 0) {
        ...
        for (let i = cu; i < units; i++)
          this._bits[i] = parts.man.rightShift(cbit + (i - cu) * DB)
                               .toUint32() & DM;
        if (cbit > 0)
  ...
  public static add (
    x: number | BigInteger, y: number | BigInteger): BigInteger {
  ...
  private static addTo (
    x: Array<number>, y: Array<number>, r: Array<number>): void {
  ...
  public static compare (
    x: number | BigInteger, y: number | BigInteger): number {
    let bi_x = typeof x === "number" ? new BigInteger(x) : x;
    let bi_y = typeof y === "number" ? new BigInteger(y) : y;
  ...
  private static create (
    sign: number, bits: number[], clamp = false): BigInteger {
  ...
  public static divRem (
    x: number | BigInteger,
    y: number | BigInteger
  ): { result: BigInteger, remainder: BigInteger } {
    let bi_x = typeof x === "number" ? new BigInteger(x) : x;
    let bi_y = typeof y === "number" ? new BigInteger(y) : y;
    if (bi_y._sign == 0) throw new RangeError();
    if (bi_x._sign == 0) return {
      result: BigInteger.Zero,
      remainder: BigInteger.Zero
    };
    if (bi_y._sign == 1 && bi_y._bits == null) return {
      result: bi_x,
      remainder: BigInteger.Zero
    };
    if (bi_y._sign == -1 && bi_y._bits == null) return {
    ...
    let c = BigInteger.compareAbs(bi_x, bi_y);
    if (c == 0) return {
      result: sign_result > 0
        ? BigInteger.One
        : BigInteger.MinusOne, remainder: BigInteger.Zero
    };
    ...
    for (let i = bi_x._bits.length - 1; i >= bi_y._bits.length - 1; i--) {
      ...
      while (min != max) {
        ...
        BigInteger.multiplyTo(
          bi_y._bits, [bits_result[offset]], bits_sub, offset);
    ...
    return {
      result: BigInteger.create(sign_result, bits_result, true),
      remainder: BigInteger.create(bi_x._sign, bits_rem, true)
    };
  }

  public static equals (
    x: number | BigInteger, y: number | BigInteger): boolean {
    let bi_x = typeof x === "number" ? new BigInteger(x) : x;
    let bi_y = typeof y === "number" ? new BigInteger(y) : y;
  ...
  public static fromUint8Array (
    arr: Uint8Array, sign = 1, littleEndian = true): BigInteger {
    let bi = Object.create(BigInteger.prototype) as BigInteger;
  ...
  private fromUint8Array (
    arr: Uint8Array, sign = 1, littleEndian = true): void {
    if (!littleEndian) {
    ...
    for (let i = 0; i < units; i++) {
      ...
      cb %= 8;
      this._bits[i] = ((arr[cu] | arr[cu + 1] << 8 | arr[cu + 2] << 16 |
        arr[cu + 3] << 24) >>> cb) & DM;
    }
  ...
  public leftShift (shift: number): BigInteger {
    ...
    if (shift == 0) {
      ...
      for (let i = shift_units; i < bits_new.length; i++)
        bits_new[i] = (this._bits[i - shift_units] << shift |
          this._bits[i - shift_units - 1] >>> (DB - shift)) & DM;
      bits_new[bits_new.length] = this._bits[this._bits.length - 1] >>>
        (DB - shift) & DM;
    }
  ...
  public static modInverse (
    value: number | BigInteger, modulus: number | BigInteger): BigInteger {
  ...
  public static modPow (
    value: number | BigInteger, exponent: number | BigInteger,
    modulus: number | BigInteger
  ): BigInteger {
    let bi_v = typeof value === "number" ? new BigInteger(value) : value;
    let bi_e = typeof exponent === "number"
  ...
  public modPow (
    exponent: number | BigInteger, modulus: number | BigInteger): BigInteger {
  ...
  public static multiply (
    ...
    return BigInteger.create(
      (bi_x._sign > 0) == (bi_y._sign > 0) ? +1 : -1, bits_r);
  }
  ...
  private static multiplyTo (
    x: Array<number>, y: Array<number>, r: Array<number>, offset = 0): void {
    if (x.length > y.length) {
      let t = x;
      x = y;
      y = t;
  ...
  public static pow (value: number | BigInteger, exponent: number): BigInteger {
    ...
    if (bi_v._bits.length == 1) {
      ...
      if (bi_v._bits[0] == -1) return (exponent & 1) != 0
        ? bi_v
        : BigInteger.One;
    }
  ...
  private static subtractTo (
    x: Array<number>, y: Array<number>, r?: Array<number>): boolean {
    if (r == null) r = [];
  ...
  public toUint8Array (littleEndian = true, length?: number): Uint8Array {
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      cbits %= DB;
      if (DB - cbits < 8)
        array[offset] = (this._bits[cu] >>> cbits |
          this._bits[cu + 1] << (DB - cbits)) & 0xff;
      else
