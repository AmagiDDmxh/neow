...
export class ECDsa {
  ...
  private static calculateE (
    n: BigInteger, message: ArrayBuffer | ArrayBufferView): BigInteger {
    return BigInteger.fromUint8Array(
      new Uint8Array(Sha256.computeHash(message)), 1, false);
  }
  ...
  public sign (message: ArrayBuffer | ArrayBufferView): ArrayBuffer {
    ...
    do
    {
      ...
      do
      {
        ...
        {
          k = BigInteger.random(
            this.key.publicKey.curve.N.bitLength(), window.crypto);
        }
      ...
      while (r.sign() == 0);
      s = k.modInverse(this.key.publicKey.curve.N)
           .multiply(e.add(d.multiply(r)))
           .mod(this.key.publicKey.curve.N);
  ...
  private static sumOfTwoMultiplies (
    P: ECPoint, k: BigInteger, Q: ECPoint, l: BigInteger): ECPoint {
  ...
  public verify (
    ...
    if (r.compareTo(this.key.publicKey.curve.N) >= 0 ||
      s.compareTo(this.key.publicKey.curve.N) >= 0)
      return false;
    let e = ECDsa.calculateE(this.key.publicKey.curve.N, message);
    ...
    let point = ECDsa.sumOfTwoMultiplies(
      this.key.publicKey.curve.G, u1, this.key.publicKey, u2);
